...
public class PlayerAI {
	...
	public boolean processInput() throws IOException {
		...
		if (ls.size() == 2) { // if opponent just played
			...
			board = new Board(Integer.parseInt(ls.get(0)), Integer.parseInt(ls
					.get(1)), Integer.parseInt(ls.get(2)), log);
			if ((isPlayerOne && Integer.parseInt(ls.get(3)) == 1)
					|| (!isPlayerOne && Integer.parseInt(ls.get(3)) == 2)) { // Make
																				// first
																				// move
	...
	/*
	 * Uses the alpha beta search algorithm to select the best move for our
	 * player
	 */
	/**
	 * @param currentBoard
	...
	Move alphaBetaSearch(Board currentBoard) {
		...
		for (Move i : currentMoves) { // for each legal move
			...
			if (currentBoard.isConnectN() != -1 || currentBoard.isFull()) { // if
																			// termainal
																			// case
				score = Eval.eval(currentBoard); // evaluate board
			} else
				score = min(nextMove, 1, alpha, beta)
						- Math.abs(i.column - currentBoard.center);
			if (v < score) {
				v = score;
				index = i;
			}
	 ...
	 * @param currentDepth
	 *            The current depth traversed
	 * @param alpha
	 *            The highest value so far used for alpha beta pruning
	 * @param beta
	 *            The lowest value so far used for alpha beta pruning
	...
	int max(Board currentBoard, int currentDepth, int alpha, int beta) {
		if (currentDepth == MAX_DEPTH || currentBoard.isConnectN() != -1
				|| currentBoard.isFull()) {
			return (int) (Eval.eval(currentBoard) * (1 - (double) (currentDepth - 1)
					/ MAX_DEPTH));
		}
	...
	public static void main(String[] args) throws IOException {
		...
		ai.setName("BuschSanchezAI");
		System.out.println(ai.playerName);
		boolean gameOver = false;
		while (!gameOver)
			gameOver = ai.processInput();
	}
}
